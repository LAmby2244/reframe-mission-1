 ...
 * Vercel Edge Function — WhatsApp Balcony Practice Nudge
...
export default async function handler(req) {
  ...
  try {
    ...
    if (!settings.length) return new Response(JSON.stringify({ sent: 0 }), { headers: { 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ sent, checked: settings.length }), {
